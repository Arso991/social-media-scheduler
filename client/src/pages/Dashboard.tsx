...
import type { Account, Activity, Post } from "../types/types";
    ...
    const fetchDashboardData = async () => {
      try {
        const [postsRes, accountsRes, activityRes] = [
          { data: dummyPostsData as Post[] },
          { data: dummyAccountsData as Account[] },
          { data: dummyActivityData as Activity[] },
        ];
        setStats({
          scheduled: postsRes.data.filter((p) => p.status === "scheduled")
            .length,
          published: postsRes.data.filter((p) => p.status === "published")
            .length,
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {statCards.map((card, key) => (
          <div
            key={key}
            className="bg-white hover:bg-red-50 relative border border-slate-200 rounded-2xl p-5
