...
const AIComposer = () => {
  ...
  const [generations, setGenerations] = useState<Post[]>([]);

  //Scheduling state
  const [activeScheduler, setActiveSceduler] = useState<Post | null>(null);
    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="space-y-6 text-center mb-6">
        ...
        <div className="relative group mt-12">
          <textarea
            className="w-full p-6 bg-white border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400 outline-none
          focus:border-slate-400 transition resize-none h-40"
            placeholder="Share your idea... (e.g. A post about the launch og our new eco-friendly coffee beans)"
            value={prompt}
        ...
        <div className="fixed inset-0 min-h-screen z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-8 py-4 border-b border-slate-100 bg-slate-50/30">
              ...
              <button
                onClick={() => setActiveSceduler(null)}
            ...
            <div className="p-8 bg-slate-50/50 border-t border-slate-50 space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="relative">
                    <CalendarIcon className="size-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-auto" />
                    <input
                      type="date"
                      required
                  ...
                  <div className="relative">
                    <ClockIcon className="size-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-auto" />
                    <input
                      type="time"
                      required
                      className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-md text-slate-900 text-sm focus:outline-none transition-all"
              ...
              <button
                ...
                disabled={loading}
                className="w-full flex items-center justify-center gap-2 py-3 bg-slate-200 text-slate-700 hover:bg-red-500 transition hover:text-white rounded-md"
              >
                {scheduling ? (
                  <Loader2Icon className="size-4 animate-spin" />
                ) : (
