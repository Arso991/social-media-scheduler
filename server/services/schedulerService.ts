  ...
  cron.schedule("* * * * *", async () => {
    try {
      ...
      for (const post of postsToPublish) {
        try {
          const accounts = await Account.find({
            // cast filter to any to satisfy TypeScript when 'user' isn't present
            // on the Account query type while still querying by user id.
      ...
      if (postsToPublish.length > 0) {
        console.log(
          `Evaluated ${postsToPublish.length} post at ${now.toISOString()}`,
        );
      }
    } catch (error) {
